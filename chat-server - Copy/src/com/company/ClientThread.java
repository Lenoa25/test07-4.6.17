...
public class ClientThread extends Thread {
    ...
    public ClientThread(Socket clientSocket, List<Message> messages, Map<String, String> users,FileInformation file) {
    ...
    private void upload() throws IOException {
        InputStream inputStreamUpload = clientSocket.getInputStream();
        OutputStream outputStreamUpload = new FileOutputStream(file.file);
        byte[] buffer = new byte[2048];

        int length = 0;

        while ((length = inputStreamUpload.read(buffer)) != -1) {
            System.out.println("Buffer Write of length: " + length);
            outputStreamUpload.write(buffer, 0, length);
        }
        inputStreamUpload.close();
        outputStreamUpload.close();
    }

    private void download() throws IOException {
        InputStream inputStreamUpload = new FileInputStream(file.file);
        OutputStream outputStreamUpload = clientSocket.getOutputStream();
        int lastMessageReceived=-1;
        lastMessageReceived=inputStreamUpload.read();
        if(lastMessageReceived-1==file.version)return;
        byte[] buffer = new byte[2048];
        int length = 0;
        while ((length = inputStreamUpload.read(buffer)) != -1) {
            System.out.println("Buffer Write of length: " + length);
